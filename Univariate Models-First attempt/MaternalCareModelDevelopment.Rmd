...
### Z-transform all continuous variables

I already transformed cortisol for early model development. Now I need to trandform the others

```{r}


Zcort <- standardize(DFbinom$MatCort)
Zage <- standardize(DFbinom$age)
Zlitsize <- standardize(DFbinom$LitSize)
Z13C <- standardize(DFbinom$d13C)
Z15N <- standardize(DFbinom$d15N)
Zmatcond <- standardize(DFbinom$MatCond)
Zmatcort <- standardize(DFbinom$MatCort)
Zlitcond <-as.numeric(standardise_aes_names(DFbinom$LitCond))
...
LMC = b0+ b1 x Cortisol + b2 x N15 + b3 x C13+ b4 x MaternalBC + b5 x LitterSize + b6 x LitterBC + b7 x age + b8 x (MaternalBC x LiiterSize) +(1|BearID) + (1|year)
...
ReBinomMC.Fulldata <- list(N=N, y=y,Zcort=Zcort,MP=MP, Nbear=Nbear,
                           bear=bear,Z15N=Z15N, Z13C=Z13C, Zmatcond=Zmatcond,
                           Zlitsize=Zlitsize, Zlitcond=Zlitcond, Zage=Zage)
str(ReBinomMC.Fulldata)

wanted.Full <- c("bCORT", "bMP", "b15N", "b13C", "bMatBC", "bLitSize",
            "bLitBC", "bAge", "bMBCxLS","pMean", "mu", "sigma", "b0")
...
ReJagsBinomMC.Full <- jags(ReBinomMC.Fulldata,NULL,wanted.Full,
                    model="ReBinomMCmodel_Full.txt",
...
ReJagsBinomMC.Full_output <- mcmcOutput(ReJagsBinomMC.Full)

J <- summary(ReJagsBinomMC.Full_output)
K <- as.data.frame(J[1:9,])
print(K)

write.csv(K,"LMC_JagsFullModelOutput.csv")

Test <- read.csv("LMC_JagsFullModelOutput.csv")

diagPlot(ReJagsBinomMC.Full_output)

postPlot(ReJagsBinomMC.Full_output)

autocorr.diag(as.mcmc(ReJagsBinomMC.Full_output))
