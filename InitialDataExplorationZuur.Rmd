...
bear.reprodata$reprofinal <- as.factor(bear.reprodata$reprofinal)
...
dotchart(bear.reprodata$N15,
         groups = factor(bear.reprodata$reprofinal),
         ylab= "Reproductive Category",
         xlab="Delta 15N",
         main="Cleveland Dotplot",
         color = bear.reprodata$reprofinal)
...
# Cortisol is a different story from 15N. There are several outliers in the Soliary female category and one in the WithCubs category. All three categories have similar spreads, but there does seem to be some structure to this, possibly violating our assumptions of homogeneity.I think its a bit skewed
...
ggplot(data = bear.reprodata, aes(reprofinal,N15))+
  geom_boxplot(aes(group=reprofinal, fill=reprofinal))+
  scale_fill_viridis(discrete = TRUE)+
  theme_classic()
# Again, we see some indications that there are a few outliers.

ggplot(data = bear.reprodata, aes(reprofinal,cortisol.kitcorrected))+
  geom_boxplot(aes(group=reprofinal, fill=reprofinal))+
  scale_fill_viridis(discrete = TRUE)+
  theme_classic()
# Here we see some outliers, mostly grouped in the Solitary female category

ggplot(data = bear.reprodata, aes(reprofinal,condition))+
  geom_boxplot(aes(group=reprofinal, fill=reprofinal))+
  scale_fill_viridis(discrete = TRUE)+
  theme_classic()
