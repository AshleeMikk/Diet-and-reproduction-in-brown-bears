...
So, bears with higher $\delta^{15}N$ signatures tend to also have lower cortisol, and the relationship is steeper with lower $\delta^{15}N$ signatures than higher signatures. Bears with higher $\delta^{15}N$ signatures tend to be older, larger, males who (supposedly) have higher metabolic demands. So I can interpret this relationship 2 ways:

1: eating more meat or protein rich foods is a better diet for bears and bears who eat these foods are in better condition and have fewer metabolic demands than other bears.

2: Bears with lower metabolic demands or that are in better condition can afford to seek out and consume a diet with more protein-rich foods.

I will need to see how this relationship shakes out in the PCA and PLSR.


```{r, include=FALSE}
INT.d15N <- RC.d15N <- glmer(N15 ~1+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))
INT.d15N.aicc <- AICc(INT.d15N)

RC.d15N <- glmer(N15 ~reprocat+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))
RC.d15N.aicc <- AICc(RC.d15N)
...
**NOTE**: There are no males in this analysis. But based on a GLMM, females with dependent offspring had lower $\delta^{15}N$ values than solitary females 
...
```{r, include=FALSE}
UnZCond <- (NonLinear.Cond*sd(Cort.Repro$condition))+
...
ggplot(data = df.CondQuad, aes(UnZCond,pred.cort.QuadCond))+
  ...
  scale_y_continuous(expand=c(0,0))+
  scale_x_continuous(expand = c(0,0))+
  geom_vline(xintercept = 1.485, lwd=1,
             color="grey77", lty="dashed")+
  geom_text(label="1.485", y=0.04, x=1.52, size=5, color="grey57")
...
We see that the regression predicts that cortisol is greatest in bears of median body condition, and as body condition increases, cortisol decreases. This is likely due to these bears having fewer metabolic demands or greater resources to meet metabolic demands. Bears below the median body condition also have lower cortisol concentrations. This is likely driven by a down-regulation of cortisol, whether it's reduced APH reactivity or binding molecules. This could be a result of the poor body condition decreasing activity, or may indicate bear with the greatest metabolic demands (likely lactating females) down regulating cortisol, perhaps related to reproduction.

```{r}
INT.BC <- glmer(condition ~1+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))
INT.BC.aicc <- AICc(INT.BC)

RC.BC <- glmer(condition ~reprocat+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))

RC.BC.aicc <- AICc(RC.BC)
...
Cort.Repro$fAGE <- as.factor(Cort.Repro$age)

```

According to the GLMM body condition ~ reproductive category, females with dependent offspring have higher body condition than solitary females

```{r,fig.height=6, fig.width=7}

ggplot(data = Cort.Repro, aes(condition))+
  geom_histogram(aes(fill=reprocat), 
                 binwidth = 0.5*sd(Cort.Repro$condition))+
  scale_fill_manual(values =c(P[1],P[4]),
                    breaks = c("Sfem", "Wcubs"),
                    labels=c("Independent Females", 
                             "Females with offspring"))+
  labs(fill="Reproductive Category")+
  scale_y_continuous(expand = c(0,0))+
  scale_x_continuous(expand=c(0,0))

```

When we look at the distribution of body condition in the two different reproductive categories, females with offspring tend to be at the higher end of the body condition distribution, which makes some sense, because bears need to meet a threshold of ~20% fat in order to reproduce, so only females in generally "good" condition will reproduce. However, this applies to the fall body condition, and we measure bears in the spring after den emergence. Therefore, I expected females with dependent offspring to be in worse body condition after giving birth and lactating (if she is with cubs of the year) or trading off safety for foraging to avoid males and protect cubs (if cub age is beyond cubs of the year).
...
Quadagelm<- glmer(cort ~Zage+Zage^2+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"),
...
print(lnageaic)
```
and log linear is the best-supported relationship, giving me the underlying model:
...
My models are getting warning messages for singularity and when I look at the output, BearID is accounting for 0 variation. So, below I dig into this further and reach a final decision on whether or not to include BearID as a random effect.
...
RE00 <- lm(lnCORT~1, data = Cort.Repro)
RE00.aicc <- AICc(RE00)

RE0 <- lm(lnCORT~bearID, data = Cort.Repro)
RE0.aicc <- AICc(RE0)
...
```
When I run a simple regression on the log-transformed cortisol data and compare a true "intercept only model" with a model that looks at the effect of bearID, the bearID model has a $\Delta AIC_{c}$ = 101.4 units **higher** than the intercept only model

##### comparing random effect structures


```{r}
RE1<- glmer(cort ~1+(1|bearID),
               data = Cort.Repro,
               family = gaussian(link = "log"),
...
ll <- getME(RE1,"lower")
min(tt[ll==0])
# It doesn't seem like singularity is a problem in this case

RE1<- blmer(cort ~1+(1|bearID),
               data = Cort.Repro)
summary(RE1)


RE2<- glmer(cort ~1+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))
RE2.aicc <- AICc(RE2)

RE3<- glmer(cort ~1+(1|bearID)+(1|fYR),
               data = Cort.Repro,
               family = gaussian(link = "log"))
RE3.aicc <- AICc(RE3)
